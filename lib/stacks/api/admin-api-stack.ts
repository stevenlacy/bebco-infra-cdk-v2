...
export class AdminApiStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AdminApiStackProps) {
    ...
    const openApiSpecPath = path.join(__dirname, '../../../exports/api-configs/swagger/bebco-staging-admin.json');
    const openApiSpec = JSON.parse(fs.readFileSync(openApiSpecPath, 'utf-8'));
    
    // Update the spec for dev environment
    openApiSpec.info.title = 'bebco-admin-dev-api';
    ...
    this.api = new apigateway.SpecRestApi(this, 'AdminApi', {
      restApiName: resourceNames.apiGateway('admin-api'),
    ...
    new cdk.CfnOutput(this, 'ApiEndpoint', {
      value: this.api.url,
      description: 'Admin Portal API endpoint',
    });
    
    new cdk.CfnOutput(this, 'ApiId', {
      value: this.api.restApiId,
      description: 'Admin Portal API ID',
    });
    ...
    cdk.Tags.of(this).add('API', 'admin-portal');
  }
